#!/usr/bin/env bash
# profile_stream_open.sh - produce flamegraph + perf-stat for stream_open_bench.
set -euo pipefail
...
echo "=== Building stream_open_bench in release mode ==="
unset CARGO_TARGET_DIR
cargo build --release --bin stream_open_bench

OUTDIR="bench-results/profile-stream-open/$BRANCH"
mkdir -p "$OUTDIR"
cp target/release/stream_open_bench "$OUTDIR/stream_open_bench"
...
perf stat -e "$PERF_EVENTS_CSV" -- target/release/stream_open_bench "${SO_ARGS[@]}" \
